...
`App.usage.add(tokens, mode)` пишет расход в `localStorage` по дате.
Курс и нейронка разделены полем `ai` — в тарифах без него лимита нет вообще:

| Тариф | Цена | Нейронка | В день | Режимы |
|---|---|---|---|---|
| Бесплатный | 0 ₽ | нет | — | — |
| Полный курс | 490 ₽/мес | нет | — | — |
| Максимум | 990 ₽/мес | есть | 100 000 | эконом, стандарт, полный агент |

Правила, которые проверяет `tools/smoke-test.js`:

- `App.planAI()` — входит ли нейронка в текущий тариф;
- `App.canUseMode(id)` при отсутствии нейронки возвращает `{ ok: false, why: "noai" }`,
  и `App.ai.ask()` отвечает ошибкой с кодом `NOAI` и подсказкой `App.noAiHint()`;
- `App.recommendMode()` и `App.ai.mode()` возвращают `null`, если режимов нет;
- `ST.bonusTokens()` равен нулю там, где нейронки нет, — бонус за прогресс нечего расходовать;
- `App.aiRing()` рисует кружок только у тарифов с нейронкой, иначе показывает прочерк «—».

Режим трат выше доступного блокируется с предложением тарифа, в котором он есть.
