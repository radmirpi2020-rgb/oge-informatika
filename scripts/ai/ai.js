...
(function () {
  ...
  AI.systemPrompt = function (modeId) {
    ...
    var plan = ST.planDef();
    var lines = [];
    lines.push("Ты — «Провожатый», помощник школьника по информатике. Учишь мягко, по делу, без лекций.");
    lines.push("На сайте два отдельных курса: «Информатика 5–9 класс» (школьная программа, 300 уроков и практик) и «Подготовка к ОГЭ» (экзамен: 16 заданий, 21 первичный балл, 150 минут).");
    lines.push("Помогай с тем уроком, который открыт, и держись курса ученика: в школьном курсе объясняй тему, в курсе ОГЭ — ещё и требования критериев.");
    lines.push("В курсе ОГЭ у каждого урока есть режим: симуляция в сайте, «скачать файл», «вживую в настоящей программе» (LibreOffice, Кумир, Python) и «полный вариант на 150 минут».");
    ...
    if (ctx.lesson) {
      ...
      lines.push("");
      lines.push("Ученик сейчас на элементе №" + l.id + " «" + l.title + "» (курс «" + App.course.label(App.course.ofLesson(l)) +
        "», модуль «" + (l.module || "") + "», " +
        (l.kind === "practice" ? "практика" : "урок") + ").");
      if (l.sub) lines.push("Тема: " + l.sub + ".");
      if (l.oge) {
        lines.push("Это курс ОГЭ: задание " + l.oge.exam + ", балл " + l.oge.ball + ", режим урока — " + l.oge.modeLabel + ".");
        if (l.oge.files && l.oge.files.length) {
          lines.push("Файлы урока: " + l.oge.files.map(function (f) { return f.title + " (" + f.size + ")"; }).join("; ") + ".");
        }
        if (l.oge.realRuns) lines.push("Минимум живых прогонов по модулю: " + l.oge.realRuns + ".");
        if (l.oge.check) lines.push("Модуль закрыт, когда: " + l.oge.check + ".");
      }
